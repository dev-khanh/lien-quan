...
export function RentForm({ accountId }: { accountId: string }) {
  ...
  async function submit(formData: FormData) {
    ...
    setMessage(res.ok ? "Đã tạo đơn. Admin sẽ xác nhận thanh toán trước khi giao acc." : data.error || "Có lỗi xảy ra.");
  }
  return (
    <form action={submit} className="space-y-3 rounded-lg bg-white p-4 shadow-game">
      ...
      <button className="w-full rounded-md bg-pink-600 px-4 py-2 font-black text-white">Gửi đơn thuê</button>
